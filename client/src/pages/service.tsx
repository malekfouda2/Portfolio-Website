...
export default function ServicePage() {
  ...
  return <MarketingLayout>
    ...
    <header className="marketing-hero"><div className="marketing-shell"><Link href="/services" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-green-400"><ArrowLeft className="h-4 w-4" />All services</Link><div className="mt-12 grid gap-10 lg:grid-cols-[1fr_0.45fr] lg:items-end"><div><p className="marketing-eyebrow">{service.eyebrow}</p><h1 className="marketing-title">{service.title}</h1></div><div><p className="text-lg leading-8 text-gray-300">{service.shortDescription}</p><a href="https://calendly.com/malekfouda2000/30min" target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("service_inquiry_click", "lead", service.slug)} className="primary-cta mt-7">Book a Call <ArrowUpRight className="h-4 w-4" /></a></div></div></div></header>
    ...
    <section className="marketing-section"><div className="marketing-shell"><div className="marketing-card p-8 text-center sm:p-12"><h2 className="text-3xl font-bold sm:text-4xl">Tell me what needs to work better.</h2><div className="mt-8 flex flex-wrap justify-center gap-4"><a href="https://calendly.com/malekfouda2000/30min" target="_blank" rel="noopener noreferrer" className="primary-cta">Book a Call <ArrowUpRight className="h-4 w-4" /></a><Link href="/contact" className="secondary-cta">Send the details <ArrowRight className="h-4 w-4" /></Link></div></div></div></section>
  </MarketingLayout>;
}
