  ...
  return <footer className="border-t border-gray-800 bg-gray-950 py-12">
    <div className="container mx-auto px-4 sm:px-6"><div className="mx-auto max-w-6xl">
      <div className="grid gap-10 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <a href={getCalendlyUrl("footer")} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("calendly_click", "lead", "footer")} className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-green-400 to-blue-500 px-6 py-3 font-semibold text-black transition hover:scale-105">Book a Call <ArrowUpRight className="h-4 w-4" /></a>
      </div>
      <div className="mt-10 flex flex-col gap-6 border-t border-gray-800 pt-8 text-sm text-gray-400 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-5"><Link href="/services" className="hover:text-green-400">Services</Link><Link href="/solutions" className="hover:text-green-400">Solutions</Link><Link href="/portfolio" className="hover:text-green-400">Projects</Link><Link href="/work" className="hover:text-green-400">Work</Link><Link href="/about" className="hover:text-green-400">About</Link><Link href="/contact" className="hover:text-green-400">Contact</Link><Link href="/privacy" className="hover:text-green-400">Privacy</Link><button type="button" onClick={() => window.dispatchEvent(new Event("open-cookie-settings"))} className="hover:text-green-400">Cookie settings</button></div><div className="flex flex-wrap gap-5"><a href={`mailto:${SITE_IDENTITY.email}`} className="hover:text-green-400">Email</a><a href={SITE_IDENTITY.github} target="_blank" rel="noopener noreferrer" className="hover:text-green-400">GitHub</a><a href={SITE_IDENTITY.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-green-400">LinkedIn</a><a href={getWhatsAppUrl("footer")} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("whatsapp_click", "lead", "footer")} className="hover:text-green-400">WhatsApp</a></div></div>
