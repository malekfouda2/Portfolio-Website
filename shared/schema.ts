...
export const insertContactSchema = createInsertSchema(contacts)
  ...
  .extend({
    ...
    company: z.string().min(2).max(120).nullable().optional(),
    websiteUrl: z.string().max(300).nullable().optional(),
    projectType: z.string().min(2).max(80).nullable().optional(),
    message: z.string().min(10).max(2000),
    budgetRange: z.string().min(2).max(80).nullable().optional(),
    timeline: z.string().min(2).max(80).nullable().optional(),
    preferredContact: z.enum(["email", "calendly", "whatsapp"]).nullable().optional(),
